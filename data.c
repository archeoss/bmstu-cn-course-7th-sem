...
enum status data_prepare_dirlisting_buf(const struct response *res,
...
enum status data_prepare_error_buf(const struct response *res,
                                   struct buffer *buf, size_t *progress) {
  /* reset buffer */
  memset(buf, 0, sizeof(*buf));
  ...
  if (*progress == 0) {
    ...
    if (buffer_appendf(buf,
                       ...
                       "\t<body>\n\t\t<h1>%d %s</h1>\n"
                       "\t</body>\n</html>\n",
                       res->status, status_str[res->status], res->status,
...
enum status data_prepare_file_buf(const struct response *res,
                                  struct buffer *buf, size_t *progress) {
  FILE *fp;
  enum status s = 0;
  ssize_t r;
  ...
  while ((r = fread(buf->data + buf->len, 1,
                    MIN(sizeof(buf->data) - buf->len, remaining), fp))) {
    if (r < 0) {
      s = S_INTERNAL_SERVER_ERROR;
      if (fp) {
        fclose(fp);
